...
import { residentialServiceDetails } from "@/lib/data";
...
function ServiceSection({ svc, index }: { svc: typeof residentialServiceDetails[number]; index: number }) {
  const ref = useInView();
...
export default function ServiceCategoryClient() {
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.svcHeroSplit} ref={heroRef.ref}>
        <div className={s.svcHeroText}>
          <div className={s.svcBreadcrumb}>
            ...
            <span>/</span>
            <span style={{ color: "var(--accent)" }}>Residential</span>
          </div>
          ...
          <p style={fadeUpCoolors(heroRef.visible, 100)} className={s.svcHeroDesc}>
            Expert electrical solutions for your home. From lighting and panel upgrades to complete rewiring, our licensed electricians deliver safe, code-compliant work across the GTA.
          </p>
        ...
        <div className={s.svcHeroImage}>
          <Image src="/images/residential-hero.jpg" alt="Residential electrician at work" fill sizes="50vw" style={{ objectFit: "cover" }} priority />
        </div>
      </div>

      <section className={s.section}>
        <div className={s.sectionInner}>
          {residentialServiceDetails.map((svc, i) => (
      ...
      <section className={`${s.section} ${s.sectionBgPale}`}>
        <div className={s.sectionInner} style={{ textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
            <Link href="/services/commercial" className={s.btnPrimary} style={{ display: "inline-block" }}>Commercial Services</Link>
      ...
      <CTA title="Need Residential Electrical Work?" desc="Free estimates on all residential projects. Same-day service available across the GTA." />
